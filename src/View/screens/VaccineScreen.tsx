...
export function VaccineScreen({ navigation }) {
  ...
  async function criarCarteira(){
    try{
      const cadastrarCarteira = await CarteiraController.cadastrarCarteira(usuarioSessao);
  ...
  async function criarCarteiraDependente(idDependente: number){
    try{
      const cadastrarCarteiraDependente = await carteiraDependenteController.cadastrarCarteiraDependentes(idDependente);
      setIsModalVisible(false);
      fetchData();
    }catch(error){
      throw error;
  ...
  const fetchData = async () => {
    const carteiraUsuario = await CarteiraController.buscarCarteira(usuarioSessao);
    console.log("Carteira usuario" + carteiraUsuario);
  
    const carteirasDependentes = await carteiraDependenteController.buscarCarteirasDependentes(usuarioSessao);
  ...
  const renderCarteiraUsuario = () => {
    const carteiraUsuario = carteiras.length > 0 ? carteiras[0].find(carteira => carteira && carteira.id_carteira) : null;
    ...
    return (
      <View>
        ...
        {carteiraUsuario && (
          <TouchableOpacity key={carteiraUsuario.id_carteira} onPress={() => navigation.navigate("VaccinesUser", { id_carteira: Number(carteiraUsuario.id_carteira) })}>
            {carteiraUsuario.id_carteira && <Text>ID: {carteiraUsuario.id_carteira}</Text>}
            {carteiraUsuario.id_usuario && <Text>ID do usuario: {carteiraUsuario.id_usuario}</Text>}
          </TouchableOpacity>
        )}
      </View>
    );
  ...
  const renderCarteirasDependentes = () => {
    const carteirasDependentes = carteiras.filter(carteira => carteira.id_carteiraDependente);
  
    return (
      <View>
        <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Carteiras de Dependentes:</Text>
        {carteirasDependentes.map(carteira => (
          <TouchableOpacity key={carteira.id_carteiraDependente} onPress={() => navigation.navigate("VaccinesDependent", { id_carteiraDependente: carteira.id_carteiraDependente })}>
            <Text>ID da Carteira Dependente: {carteira.id_carteiraDependente}</Text>
