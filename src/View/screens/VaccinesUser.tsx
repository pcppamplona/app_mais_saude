...
export function VaccinesUser({ route, navigation }){
    ...
    const openEditarModal = (id_vacina, Nome_Vacina, Local_Vacinacao, Data_Vacinacao, Dose) => {
    ...
    async function cadastrarVacina(nomeVacina, localVacinacao, dataVacinacao, dose){
        ...
        try{
            const cadastraVacina = await VacinaController.cadastrarVacina(id_carteira, nomeVacina, localVacinacao, newData, Number(dose))
            await buscarVacinas()
        }catch(error){
            throw error;
        }
    ...
    async function editarVacina(idVacinaDependente, NomeVacina, LocalVacinacao, DataVacinacao, Dose){
        ...
        try{
            const editarVacina = await VacinaController.editarVacina(id_carteira, idVacinaDependente ,NomeVacina, LocalVacinacao, DataVacinacao, Dose)
            await buscarVacinas()
        }catch(error){
            throw error;
        }
    ...
    async function excluirVacina(idVacina){
        try{
            ...
            const excluirVacina = await VacinaController.excluirVacina(id_carteira, idVacina)
            await buscarVacinas()
        }catch(error){
            throw error;
        }
    ...
    const renderItem = ({ item }) => (
        <View style={{ marginBottom: 16, borderBottomWidth: 1, paddingBottom: 8 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }}>
            <TouchableOpacity onPress={() => openEditarModal(item.id_vacina, item.Nome_Vacina, item.Local_Vacinacao, item.Data_Vacinacao, item.Dose)}><Text>Editar</Text></TouchableOpacity>
            ...
            <Modal isVisible={isEditarModalVisible}>
                {vacinaSelecionada && (
                    ...
                    <TextInput
                      ...
                      editable={false}
                      value={vacinaSelecionada.id_vacina ? vacinaSelecionada.id_vacina.toString() : ''}
                    ...
                    <Button title="Salvar" onPress={() => editarVacina(Number(vacinaSelecionada.id_vacina), vacinaSelecionada.Nome_Vacina, vacinaSelecionada.Local_Vacinacao, vacinaSelecionada.Data_Vacinacao, Number(vacinaSelecionada.Dose))} />
