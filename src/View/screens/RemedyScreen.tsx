...
export function RemedyScreen({ navigation }) {
  ...
  const addRemedio = async (novoRemedio) => {
    try {
      ...
      const { nomeRemedio, horarioInicial, quantidadeDias, frequencia } = novoRemedio;
      const id_usuario = usuarioSessao;
  
      const CriarRemedio = await RemedioController.CriarRemedio(id_usuario, nomeRemedio, horarioInicial, quantidadeDias, frequencia);
      ...
      try {
        ...
        try {
          ...
          if (BuscarUltimoRemedio) {
            const { id_remedio, Nome_Remedio, Horario_Inicial, Frequencia, Qntd_Total } = BuscarUltimoRemedio;
            await agendarNotificacoes(id_remedio, Nome_Remedio, Horario_Inicial, Frequencia, Qntd_Total);
          } else {
  ...
  useEffect(() =>{
    const novaNotificacao = Notifications.addNotificationReceivedListener(async notification => {
  ...
  async function agendarNotificacoes(idRemedio, nomeRemedio, horarioInicial, frequencia, qntdTotalDoses) {
    ...
    for (let i = 0; i < qntdTotalDoses; i++) {
      ...
      const segundosParaProximaDose = i * (frequenciaHoras * 3600 + frequenciaMinutos * 60 + frequenciaSegundos);
  
      // Calcule o horário da próxima dose
      const proximaDose = new Date();
      proximaDose.setHours(horas, minutos, segundos);
      proximaDose.setTime(proximaDose.getTime() + segundosParaProximaDose * 1000);
  ...
  async function editarNotificacao(idRemedio: number, novoNome:string, novoHorario: string, novaqntdDias: number, novaFrequencia: string){
    console.log("Editando agendamento")
    await Notifications.cancelScheduledNotificationAsync(idRemedio.toString());
    await agendarNotificacoes(idRemedio, novoNome, novoHorario, novaFrequencia, novaqntdDias);
  }
  ...
  return (
    <View className="flex-1 items-center justify-center bg-white">
      <Text className="text-red-700">Remedios</Text>
      <TouchableOpacity onPress={toggleModal} style={{backgroundColor: 'green', // Cor de fundo do botão
    width: 40,
    height: 40,
    borderRadius: 20,
    alignItems: 'center',
    ...
    right: 20,}}>
      ...
      {remedios.length === 0 ? (
        <Text>Não há remédios adicionados.</Text>
      ) : (
